...
describe("CLI progress text", () => {
  it("formats Chinese write progress lines", () => {
    ...
    expect(
      formatWriteCompletionLines("zh", {
        ...
        revised: true,
        status: "audit-failed",
        issues: [{ severity: "warning", category: "continuity", description: "时间线略有跳变" }],
      }),
    ).toEqual([
      "  第7章：潮声夜渡",
      "  字数：2345字",
  ...
  it("formats English write progress lines", () => {
    expect(formatWriteStartLine("en", 2, 5, "demo-book")).toBe(
      '[2/5] Writing chapter for "demo-book"...',
    );
    expect(
      formatWriteCompletionLines("en", {
        chapterNumber: 7,
        title: "Harbor Wake",
        wordCount: 2310,
        passedAudit: true,
  ...
  it("formats Chinese import progress lines", () => {
    expect(formatImportDiscoveryLine("zh", 12, "demo-book")).toBe(
      "发现 12 章，准备导入到「demo-book」。",
    );
    expect(formatImportResumeLine("zh", 8)).toBe("从第 8 章继续导入。");
    expect(
      formatImportCompletionLines("zh", {
        importedCount: 12,
  ...
  it("formats English import progress lines", () => {
    expect(formatImportDiscoveryLine("en", 12, "demo-book")).toBe(
      'Found 12 chapters to import into "demo-book".',
    );
    expect(formatImportResumeLine("en", 8)).toBe("Resuming from chapter 8.");
    expect(
      formatImportCompletionLines("en", {
        importedCount: 12,
