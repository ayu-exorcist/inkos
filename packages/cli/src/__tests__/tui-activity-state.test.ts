...
describe("tui activity state", () => {
  it("maps chat-like intents to thinking", () => {
    ...
    expect(describeActivityState("chat", copy)).toMatchObject({
      label: "thinking",
      accent: WARM_ACCENT,
    });
    expect(describeActivityState("explain_status", copy)).toMatchObject({
      label: "checking",
      accent: WARM_ACCENT,
    });
  });
  ...
  it("maps writing and review intents to task-specific labels", () => {
    ...
    expect(describeActivityState("write_next", zhCopy)).toMatchObject({
      label: "写作中",
      accent: WARM_ACCENT,
    });
    expect(describeActivityState("revise_chapter", zhCopy)).toMatchObject({
      label: "审阅中",
      accent: WARM_ACCENT,
    });
    expect(describeActivityState("rewrite_chapter", zhCopy)).toMatchObject({
      label: "审阅中",
      accent: WARM_ACCENT,
    });
