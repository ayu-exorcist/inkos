  ...
  const handleImportChapters = async () => {
    ...
    try {
      const data = await fetchJson<{ importedCount?: number }>(
        `/books/${chBookId}/import/chapters`,
        {
          method: "POST",
  ...
  return (
    <div className="space-y-8">
      ...
      <div className={`border ${c.cardStatic} rounded-lg p-6 space-y-4`}>
        ...
        {status && (
          <div
            className={`text-sm px-3 py-2 rounded-lg ${status.startsWith("Error") ? "bg-destructive/10 text-destructive" : "bg-emerald-500/10 text-emerald-600"}`}
          >
            {status}
          </div>
        )}
