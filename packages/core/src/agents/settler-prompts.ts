...
=== USED_ELEMENTS ===
（分析本章消耗了哪些创作元素，供后续章节避免重复。格式：Markdown 列表）

- **场景原型**：本章使用的场景类型（如"藏书阁偷学""拍卖会捡漏""擂台比武""密室对峙"等），如果是一个常见套路，写出套路名
- **爽点类型**：本章兑现的爽点类型（如"打脸""升级突破""身份 reveal""智斗碾压""收益兑现"等）
- **钩子类型**：本章新开或推进的钩子类型（如"身世之谜""未解恩怨""隐藏势力""信息缺口"等）
- **对话套路**：本章使用的对话模式（如"先抑后扬的嘲讽""信息不对称的试探""威胁-反威胁"等），如果对话模式比较常规则不写
- **角色互动模式**：本章的角色关系变化类型（如"结盟""背叛""误解""和解""竞争"等）

注意：
- 只有"消耗了一个可以复用的元素"时才写，日常过渡、纯描写段落不写
- 每个条目控制在 10 个字以内，不要写分析，只写标签
- 如果本章主要是铺垫/过渡，可以写"（本章以铺垫为主，无显著消耗元素）"`;
}
