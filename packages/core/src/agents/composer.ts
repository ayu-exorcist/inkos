...
import { readRoleCards, parseECCMFromRole, buildECCMContext } from "../utils/outline-paths.js";
...
export async function composeGovernedChapter(
...
function deriveRetrievalHints(plan: PlanChapterOutput): string[] {
  return [plan.intent.goal, plan.intent.outlineNode, ...plan.memo.threadRefs].filter(
...
async function buildRecentChapterTrailEntries(
  ...
  const moodTrail = recentSummaries
    ...
    .map(
      (summary) =>
        `${summary.chapter}: ${summary.mood || "(none)"} / ${summary.chapterType || "(none)"}`,
    )
    .join(" | ");
  if (moodTrail) {
    entries.push({
  ...
  if (endingTrail) {
    entries.push({
      ...
      reason:
        "Show how recent chapters ended so the writer avoids structural repetition (e.g. 3 consecutive collapse endings).",
      excerpt: endingTrail,
    });
  }
...
async function buildHookDebtEntries(
  ...
  return targetHookIds.flatMap((hookId) => {
    ...
    const latestSummary = findHookSummary(
      summaries,
      hook.hookId,
    ...
    const seedBeat = seedSummary ? renderHookDebtBeat(seedSummary) : hook.notes || promise;
    const latestBeat =
      latestSummary && latestSummary !== seedSummary
        ...
        excerpt:
          language === "en"
            ? [
                ...
                latestBeat
                  ? `latest turn (ch${hook.lastAdvancedChapter}): ${latestBeat}`
                  : undefined,
              ]
                .filter(Boolean)
                .join(" | ")
...
function findHookSummary(
  ...
  if (mode === "seed") {
    return (
      hookMentions.find((summary) => summary.chapter === chapter) ??
      hookMentions.at(0) ??
      directChapterHit
    );
  }
  ...
  return (
    [...hookMentions].reverse().find((summary) => summary.chapter === chapter) ??
    hookMentions.at(-1) ??
    directChapterHit
  );
}
...
function summaryMentionsHook(
  ...
  return [summary.title, summary.events, summary.stateChanges, summary.hookActivity].some((text) =>
