...
const DIMENSION_LABELS: Record<number, { readonly zh: string; readonly en: string }> = {
  ...
  38: { zh: "角色因果一致性", en: "Causal Consistency Check" },
  39: { zh: "原创性检查", en: "Originality Check" },
  40: { zh: "信息释放节奏", en: "Information Release Rhythm Check" },
};
...
description 中要明确引用 hook_id，并把状态列中 stale / blocked 的原文标记字面抄进去。本维度只审结构，不评价伏笔文笔。`;
    ...
    case 38:
      return language === "en"
        ? "ECCM causal consistency: Every significant character action must be traceable to a formative event or decision-matrix entry in the character's ECCM profile (root/stem/branch). Flag actions that contradict the character's established survival strategies, collapse conditions, or absolute taboos. Check that dialogue vocabulary and rhetorical style match the character's education level and social class as recorded in the root layer. Verify that physiological reactions under pressure match the character's signature patterns and have a documented source."
        : "ECCM 因果一致性：每个重要角色行为必须能在该角色的 ECCM 档案（根系/茎干/枝叶）中找到可追溯的形成事件或决策矩阵条目。标记与角色已建立的生存策略、崩溃条件或绝对禁忌相矛盾的行为。检查对话词汇和修辞风格是否符合角色档案中记录的受教育程度和社会阶层。验证角色在压力下的身体反应是否与其标志性模式匹配，并有 documented 来源。";
    case 39:
      return language === "en"
    ...
    case 40:
      return language === "en"
        ...
        : "信息释放节奏（六铁则）：(1) 先谜后解——本章是否在给出答案前先建立悬念？(2) 先感后知——世界规则是否通过角色体验呈现，而非叙述者直接灌输？(3) 先深后新——是否先深化已有概念再引入新概念？(4) 新概念上限：第1章≤3个，后续章≤2个。(5) 世界观只通过角色行动/对话呈现——禁止叙述者信息倾倒。(6) 只揭示服务当前冲突所需的信息。单次违反标记为 warning；连续 3 章以上重复违反标记为 critical。";
    default:
      return "";
  }
}
