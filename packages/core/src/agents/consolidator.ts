...
export class ConsolidatorAgent extends BaseAgent {
  ...
  async consolidate(bookDir: string): Promise<ConsolidationResult> {
    ...
    const newSummaries: string[] = existingVolSummaries
      ? [existingVolSummaries.trim()]
      : ["# Volume Summaries\n"];
  ...
  private parseSummaryTable(raw: string): {
    header: string;
    rows: Array<{ chapter: number; raw: string }>;
  } {
    const lines = raw.split("\n");
    const headerLines = lines.filter(
      (l) =>
        l.startsWith("|") && (l.includes("章节") || l.includes("Chapter") || l.includes("---")),
    );
