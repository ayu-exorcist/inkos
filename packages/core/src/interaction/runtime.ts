...
function shouldWaitForHuman(automationMode: AutomationMode, request: InteractionRequest): boolean {
  const contentIntent =
...
function appendToolEvents(
  ...
  return events.reduce(
    (nextSession, event, index) =>
      appendInteractionEvent(nextSession, {
        ...event,
...
async function handleDraftLifecycleRequest(params: {
  ...
  switch (request.intent) {
    case "develop_book": {
      ...
      const draft = metadata.details?.creationDraft as
        | InteractionSession["creationDraft"]
        | undefined;
      if (!draft) {
        throw new Error(
          localize(language, {
  ...
  switch (request.intent) {
    ...
    case "continue_book": {
      ...
      const pendingDecision =
        metadata.pendingDecision ??
        buildPendingDecision(session, request, language, metadata.activeChapterNumber);
      const completed = pendingDecision
        ? {
            ...
            currentExecution:
              ...
              buildWaitingExecution(session, request, language, metadata.activeChapterNumber),
          }
        : {
    ...
    case "rewrite_chapter": {
      ...
      session = appendToolEvents(session, metadata.events);
      const pendingDecision =
        metadata.pendingDecision ?? buildPendingDecision(session, request, language, chapterNumber);
      const completed = pendingDecision
        ? {
            ...session,
            pendingDecision,
            currentExecution:
              metadata.currentExecution ??
              buildWaitingExecution(session, request, language, chapterNumber),
          }
        : {
            ...markCompleted(session),
            currentExecution: metadata.currentExecution ?? markCompleted(session).currentExecution,
    ...
    case "rename_entity": {
      ...
      const toolResult = await params.tools.renameEntity(
        bookId,
        request.oldValue,
        request.newValue,
      );
    ...
    case "update_focus": {
      ...
      session = appendToolEvents(session, metadata.events);
      const pendingDecision =
        metadata.pendingDecision ?? buildPendingDecision(session, request, language);
      const completed = pendingDecision
        ? {
            ...session,
            pendingDecision,
    ...
    case "edit_truth": {
      ...
      const toolResult = await params.tools.writeTruthFile(
        bookId,
        request.fileName,
        request.instruction,
      );
    ...
    case "explain_failure": {
      ...
      const summary =
        request.intent === "explain_failure"
          ? localize(language, {
