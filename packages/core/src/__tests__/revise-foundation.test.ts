...
describe("pipeline.reviseFoundation", () => {
  ...
  it("backs up legacy files and writes Phase 5 output", async () => {
    ...
    try {
      ...
      // New files created
      await expect(
        access(join(bookDir, "story", "outline", "story_frame.md")),
      ).resolves.not.toThrow();
      await expect(
        access(join(bookDir, "story", "outline", "volume_map.md")),
      ).resolves.not.toThrow();
      await expect(
        access(join(bookDir, "story", "roles", "主要角色", "林辞.md")),
      ).resolves.not.toThrow();
      // Backup exists
      ...
      await expect(
        access(join(bookDir, "story", backupDir!, "story_bible.md")),
      ).resolves.not.toThrow();
    } finally {
  ...
  it("revise 不重置运行时状态文件（current_state / pending_hooks / particle_ledger / subplot_board / emotional_arcs 保留章节累积）", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        "| H001 | 1 | 主线 | open | 15 | ... 推进到 15 章 ... |",
        "utf-8",
      );
      await writeFile(
        join(bookDir, "story", "particle_ledger.md"),
        "# 资源账本\n\n| 20 | 500 | 积累 | - | 10 | 510 | 第 20 章 |",
        "utf-8",
      );
      await writeFile(
        join(bookDir, "story", "subplot_board.md"),
        "# 支线\n\n| S1 | 宫廷阴谋 | ... | 5 | 18 | 13 | active | 已推到 18 章 |",
        "utf-8",
      );
      await writeFile(
        join(bookDir, "story", "emotional_arcs.md"),
        "# 情感弧线\n\n| 林辞 | 15 | 愤怒 | 发现背叛 | 8 | 上升 |",
        "utf-8",
      );
      await writeFile(
  ...
  it("Phase 5 书二次 revise 时从 outline/roles 权威源读（不把 shim 喂给 architect）", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        "## 主题与基调\n完整的段落式世界观描写，包含所有设定细节、人物关系、剧情主线。" +
          "a".repeat(5000),
        "utf-8",
      );
      await writeFile(
  ...
  it("revise 清空旧 role 文件（删除/改名角色后旧卡片不残留）", async () => {
    ...
    try {
      ...
      await writeFile(
        join(bookDir, "story", "roles", "主要角色", "要删掉的人.md"),
        "应该消失",
        "utf-8",
      );
      await writeFile(
        join(bookDir, "story", "roles", "主要角色", "要改名的A.md"),
        "改名前",
        "utf-8",
      );
      await writeFile(join(bookDir, "story", "roles", "次要角色", "老配角.md"), "老次要", "utf-8");
      await writeFile(
        join(bookDir, "story", "roles", "次要角色", "要删掉的次要.md"),
        "应该消失",
        "utf-8",
      );
      ...
      await expect(
        access(join(bookDir, "story", "roles", "主要角色", "改名后的B.md")),
      ).resolves.not.toThrow();
      // 旧的 5 个 role 文件里没出现在新输出的，**必须被清空**
      await expect(
        access(join(bookDir, "story", "roles", "主要角色", "要删掉的人.md")),
      ).rejects.toThrow();
      await expect(
        access(join(bookDir, "story", "roles", "主要角色", "要改名的A.md")),
      ).rejects.toThrow();
      await expect(
        access(join(bookDir, "story", "roles", "次要角色", "老配角.md")),
      ).rejects.toThrow();
      await expect(
        access(join(bookDir, "story", "roles", "次要角色", "要删掉的次要.md")),
      ).rejects.toThrow();
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("revise 模式 + LLM 意外输出 legacy 格式 → 抛错且不动架构稿文件", async () => {
    ...
    try {
      ...
      await writeFile(
        join(bookDir, "story", "roles", "主要角色", "原始角色.md"),
        "原始角色内容",
        "utf-8",
      );
      ...
      // revise 应该抛错
      await expect(runner.reviseFoundation("safe-book", "改")).rejects.toThrow(
        /legacy-format output.*NOT been modified/,
      );
      ...
      const storyFrame = await readFile(
        join(bookDir, "story", "outline", "story_frame.md"),
        "utf-8",
      );
  ...
  it("Phase 5 revise 备份目录带 phase5 tag 并包含 outline/ + roles/", async () => {
    ...
    try {
      ...
      await expect(
        access(join(bookDir, "story", backupDir!, "outline", "story_frame.md")),
      ).resolves.not.toThrow();
      await expect(
        access(join(bookDir, "story", backupDir!, "outline", "volume_map.md")),
      ).resolves.not.toThrow();
      await expect(
        access(join(bookDir, "story", backupDir!, "roles", "主要角色", "A.md")),
      ).resolves.not.toThrow();
    } finally {
      await rm(root, { recursive: true, force: true });
    }
